...
//  X_euler
//
//  Created by Jeannot on 08/11/2018.
...
int main(int argc, char ** argv) {
    ...
    if( PB103g(&pbR)) {
        pbR.nbClock = 0 ;
            if(strcmp("20313839404245",pbR.strRes)==0) {
                fprintf(stdout,"OK\tPB%s(%.06fs) Sol=%s \n",pbR.ident,(float)pbR.nbClock / CLOCKS_PER_SEC,pbR.strRes) ;
            } else {
                fprintf(stdout,"ERROR\tPB%s(%.06fs) Find=%s != Exp=%s\n",pbR.ident,(float)pbR.nbClock / CLOCKS_PER_SEC,pbR.strRes,"20313839404245") ;
            }
    } else {
             fprintf(stdout," BAD EXECUTION PB%s(%.06fs)\n",pbR.ident,(float)pbR.nbClock / CLOCKS_PER_SEC) ;
    }
}
